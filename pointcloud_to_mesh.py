"""XYZ -> building OBJ, version 0.5.1 (review prototype).
...
def conservative_noise_mask(p,mode='conservative'):
    """Reject only isolated density anomalies; never smooth or move input points."""
    if mode not in ('conservative','off'):raise ValueError('Unknown noise mode')
    dist,idx=cKDTree(p).query(p,k=min(13,len(p)),workers=-1)
    mean=dist[:,1:].mean(1);median=float(np.median(mean))
    mad=float(1.4826*np.median(np.abs(mean-median)))
    local=np.median(mean[idx[:,1:]],axis=1)
    spacing=float(np.median(dist[:,1]))
    isolated=(mean>median+6*max(mad,median*.1))&(mean>2.5*np.maximum(local,1e-9))&(dist[:,1]>max(3*spacing,.05))
    mask=~isolated if mode=='conservative' else np.ones(len(p),bool)
    if mask.sum()<20:mask[:]=True
    return mask,{'mode':mode,'input_points':len(p),'used_points':int(mask.sum()),'excluded_points':int((~mask).sum()),'excluded_indices':np.flatnonzero(~mask).tolist(),'suspected_isolated_points':int(isolated.sum()),'median_nearest_spacing':spacing,'rule':'12-neighbor mean > global median + 6 robust MAD AND > 2.5 local mean-distance median AND nearest distance > max(3 median spacing, 0.05); no point movement'}
...
def extract_planes(q,n,curv):
    ...
    for stage in range(12):
        ...
        for _ in range(650):
            ...
            pn=np.r_[-co[:2],1.];pn/=np.linalg.norm(pn)
            mask=(np.abs(p[:,2]-(p[:,:2]@co[:2]+co[2]))<threshold)&(np.abs(n[rem]@pn)>.94)
        ...
        for _ in range(3):
            ...
            pn=np.r_[-co[:2],1.];pn/=np.linalg.norm(pn)
            best=(np.abs(p[:,2]-(p[:,:2]@co[:2]+co[2]))<threshold)&(np.abs(n[rem]@pn)>.94)
            if best.sum()<minimum:break
        if best.sum()<minimum:break
    ...
    # Merge near-coplanar fragments only when combined observed residuals stay
    # small. A real height step must not be merged just because normals agree.
    consolidated=[]
    for co in coefs:
        pn=np.r_[-co[:2],1.];pn/=np.linalg.norm(pn)
        support=roof&(np.abs(q[:,2]-(q[:,:2]@co[:2]+co[2]))<.08)&(np.abs(n@pn)>.94)
        merged=False
        for j,old in enumerate(consolidated):
            on=np.r_[-old[:2],1.];on/=np.linalg.norm(on)
            if pn@on<np.cos(np.deg2rad(7)):continue
            oldsupport=roof&(np.abs(q[:,2]-(q[:,:2]@old[:2]+old[2]))<.08)&(np.abs(n@on)>.94)
            if not support.any() or np.quantile(np.abs(q[support,2]-(q[support,:2]@old[:2]+old[2])),.90)>.12:continue
            union=support|oldsupport;fit=np.linalg.lstsq(np.c_[q[union,:2],np.ones(union.sum())],q[union,2],rcond=None)[0]
            if np.quantile(np.abs(q[union,2]-(q[union,:2]@fit[:2]+fit[2])),.95)>.10:continue
            consolidated[j]=fit;merged=True;break
        if not merged:consolidated.append(co)
    coefs=np.array(consolidated)
    ...
    for i,co in enumerate(coefs):
        pn=np.r_[-co[:2],1.];pn/=np.linalg.norm(pn)
        support=q[(labels==i)&roof&(np.abs(n@pn)>.94)]
...
def prepare(input_path,noise_mode='conservative'):
    ...
    original=p.copy();keep,noise=conservative_noise_mask(p,noise_mode);p=p[keep]
    ...
    return dict(id=number,source=str(input_path),p=p,original=original,noise=noise,q=q,basis=basis,n=n,curv=curv,roof=roof,regions=regions,evidence=evidence,footprint=footprint,walls=walls,discarded_footprint_area=discarded_area)
...
def partition_roof(d):
    ...
    for i,j in itertools.combinations(main,2):
        ...
        if np.linalg.norm(diff[:2])>=.015 or abs(diff[2])<.15:continue
...
def make_mesh(d):
    ...
    for key,uses in edge_map.items():
        ...
        if len(uses)==1:
            ...
            # Preserve the fitted surfaces for inspection, but do not invent
            # a wall when planar adjacency is ambiguous. Validation must fail.
            unresolved.append({'uv_edge':[list(a),list(b)],'adjacent_patches':[use[0] for use in uses]})
    ...
    model={'id':d['id'],'source':d['source'],'vertices':arr.tolist(),'faces':faces,'semantics':semantics,'patch_ids':patch_ids,
           ...
           'unresolved_partition_edges':unresolved,
...
def reconstruct(input_path,output_path,ground_z=None,noise_mode='conservative'):
    started=time.perf_counter();d=prepare(input_path,noise_mode)
    if ground_z is not None:d['ground_z']=float(ground_z)
    model,_=make_mesh(d)
    model['algorithm_version']=VERSION
    model['noise_diagnostics']=d['noise']
    ...
    if model['unresolved_partition_edges']:model['warnings'].append('Ambiguous planar adjacency: unresolved boundary connections are left open for inspection; not an accepted solid')
...
def main():
    ...
    parser.add_argument('--noise',choices=['conservative','off'],default='conservative',help='Conservative isolated-point removal; validation must still use all original points')
    parser.add_argument('--version',action='version',version=VERSION)
    args=parser.parse_args()
    try:
        model=reconstruct(args.input,args.output,args.ground_z,args.noise)
