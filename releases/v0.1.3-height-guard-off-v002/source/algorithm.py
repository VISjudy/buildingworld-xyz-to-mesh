...
def planes_ransac(p,threshold=.10):
    ...
    for stage in range(8):
        ...
        for _ in range(550):
            ...
            if abs(np.linalg.det(a))<.1: continue
            coef=np.linalg.solve(a,tri[:,2])
        ...
        for _ in range(3):
            coef=np.linalg.lstsq(np.column_stack([q[best,:2],np.ones(best.sum())]),q[best,2],rcond=None)[0]
            best=np.abs(q[:,2]-(q[:,:2]@coef[:2]+coef[2]))<threshold
        # Reject nearly collinear support before extrapolating a roof plane.
        xy=q[best,:2]; spread=np.linalg.eigvalsh(np.cov(xy.T))
        if best.sum()<min_size or spread[0]<.01: break
...
def run(num):
    ...
    n,curve=normals(p)
    roofmask=(np.abs(n[:,2])>.55)&(curve<.035)&(p[:,2]>np.quantile(p[:,2],.25))
    roof=p[roofmask]
    coef,supports=planes_ransac(roof)
    support_recovery=False
    if len(coef)==0:
        # Failure-only second normal scale; never fit unfiltered walls as roofs.
        fine_n,fine_curve=normals(p,k=12)
        expanded=(np.abs(fine_n[:,2])>.55)&(fine_curve<.065)
        expanded &= p[:,2]>=np.quantile(p[:,2],.25)
        roofmask |= expanded
        roof=p[roofmask]
        coef,supports=planes_ransac(roof,threshold=.15)
        support_recovery=True
    if len(coef)==0: raise RuntimeError('No roof plane supported after bounded recovery')
    ...
    # Equalize dense sample regions while retaining every roof observation.
    span=np.ptp(roof[:,:2],axis=0)
    cell=max(float(np.max(span))/24.,.10)
    grid=np.floor((roof[:,:2]-roof[:,:2].min(0))/cell).astype(int)
    _,bins,counts=np.unique(grid,axis=0,return_inverse=True,return_counts=True)
    weights=1./counts[bins]
    best=None
    for count in range(1,len(coef)+1):
        for ids in itertools.combinations(range(len(coef)),count):
            err=np.abs(np.min(predicted[:,ids],axis=1)-roof[:,2])
            # A bounded tail term exposes roof elevations hidden by mean loss.
            score=float(np.average(np.minimum(err,.6),weights=weights)
                        +.08*np.quantile(np.minimum(err,3.),.9)+.004*count)
    ...
    footprint_rule='minimum rotated rectangle orientation + projected quantiles 0.002, 0.998'
    # A convex observed silhouette never cuts an interior hole from missing data.
    # Use only a substantial shape change; retain the rectangle for sparse data.
    hull=MultiPoint(p[:,:2]).convex_hull
    rectangle_area=Polygon(footprint).area
    if len(p)>=100 and hull.geom_type=='Polygon' and .60<hull.area/rectangle_area<.96:
        footprint=np.asarray(hull.exterior.coords)[:-1]
        footprint_rule='observed XY convex hull; interior completion prior, no inferred empty holes'
    # Match the original roof/wall assembly's counterclockwise orientation.
    signed=np.sum(footprint[:,0]*np.roll(footprint[:,1],-1)-footprint[:,1]*np.roll(footprint[:,0],-1))
    if signed<0: footprint=footprint[::-1]
    base=float(np.quantile(p[:,2],.001))
    # Height is relative to the provisional base, never to canonical Z=0.
    # Remove envelope planes that extrapolate below that base at any roof corner.
    corner_z=footprint@coef[:,:2].T+coef[:,2]
    if False and np.any(np.min(corner_z,axis=1)<=base+1e-6):  # controlled height-guard ablation
        feasible=np.all(footprint@candidate_coef[:,:2].T+candidate_coef[:,2]>base+1e-6,axis=0)
        ids=np.flatnonzero(feasible)
        if len(ids)==0: raise RuntimeError('No roof hypothesis above provisional observed base')
        best=None
        for count in range(1,len(ids)+1):
            for subset in itertools.combinations(ids,count):
                err=np.abs(np.min(predicted[:,subset],axis=1)-roof[:,2])
                score=float(np.average(np.minimum(err,.6),weights=weights)
                            +.08*np.quantile(np.minimum(err,3.),.9)+.004*count)
                if best is None or score<best[0]: best=(score,subset)
        selected=list(best[1]); coef=candidate_coef[selected]
        supports=[candidate_supports[i] for i in selected]
    ...
    metrics=dict(input_points=len(p),roof_candidate_points=len(roof),plane_supports=supports,
        ...
        footprint_rule=footprint_rule,support_recovery_used=support_recovery,
        roof_selection_rule='spatially balanced clipped residual plus bounded tail; base-relative corner guard',
        base_observation_status='unverified lowest-return completion; not terrain or full building height',height_guard_disabled_for_ablation=True,
        limitations=['Convex single-volume roof only; concave footprints/courtyards/extensions unsupported',
          'Small roof details below max(18, 3 percent) support omitted; local annexes may remain missing','Input unit assumed meter for Blender display only',
