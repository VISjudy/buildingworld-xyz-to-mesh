...
ROOT=HERE.parents[1]; OUT=ROOT/'work/experiments/lod2_xyz_detail'; OUT.mkdir(parents=True,exist_ok=True)
...
def prepare(number):
    p=np.loadtxt(ROOT/'data/inputs/LiDAR_xyz'/f'{number}.xyz'); rect=np.array(MultiPoint(p[:,:2]).minimum_rotated_rectangle.exterior.coords)[:-1]
...
def make_mesh(d):
    ...
    model={'id':d['id'],'source':str(ROOT/'data/inputs/LiDAR_xyz'/f"{d['id']}.xyz"),'vertices':arr.tolist(),'faces':faces,'semantics':semantics,'patch_ids':patch_ids,
...
def mesh_preview(all_data,models):
    ...
    for row,(d,model) in enumerate(zip(all_data,models)):
        p=d['p'];vs=np.array(model['vertices']);old=json.loads((ROOT/'work/experiments/lod2_xyz_baseline'/f"{d['id']}_model.json").read_text())
        for col in range(3):
