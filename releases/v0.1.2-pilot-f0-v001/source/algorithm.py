...
def run(num):
    ...
    # Use observed convex support instead of imposing four right-angle corners.
    # Retain the existing robust bounds and rectangle fallback for sparse support.
    footprint_support=np.unique(proj[((proj>=lo)&(proj<=hi)).all(axis=1)],axis=0)
    footprint_rule='minimum rotated rectangle + projected quantiles; sparse-support fallback'
    if len(footprint_support)>=3 and np.linalg.matrix_rank(footprint_support-footprint_support.mean(axis=0))==2:
        hull=ConvexHull(footprint_support)
        footprint=footprint_support[hull.vertices]@basis.T
        footprint_rule='convex hull of observed XY within projected quantiles 0.002, 0.998'
