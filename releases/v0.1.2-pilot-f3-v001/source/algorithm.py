...
def planes_ransac(p,threshold=.10,min_support=60):
    rng=np.random.default_rng(739)
    rem=np.arange(len(p)); planes=[]; supports=[]
    min_size=max(min_support,int(len(p)*.075))
...
def run(num):
    ...
    recovery_used=False
    recovery_min_support=None
    if len(coef)==0:
        # Retry candidate starvation only; successful baseline fits are unchanged.
        # Inclusive numerical ties retain a flat roof at the height cutoff.
        cutoff=float(np.quantile(p[:,2],.25))
        tie_tol=1e-8*max(1.,float(np.ptp(p[:,2])))
        retry_mask=(np.abs(n[:,2])>.55)&(curve<.035)&(p[:,2]>=cutoff-tie_tol)
        retry_roof=p[retry_mask]
        # Sparse scans cannot meet a fixed 60-return support floor. Retain a
        # 12-return minimum and the original 7.5-percent rule inside RANSAC.
        retry_min=max(12,min(60,int(np.ceil(.15*len(retry_roof)))))
        if len(retry_roof)>=retry_min:
            retry_coef,retry_supports=planes_ransac(retry_roof,min_support=retry_min)
            if len(retry_coef):
                roof=retry_roof; coef=retry_coef; supports=retry_supports
                recovery_used=True; recovery_min_support=retry_min
    ...
    metrics=dict(input_points=len(p),roof_candidate_points=len(roof),plane_supports=supports,
        roof_candidate_recovery_used=recovery_used,recovery_min_support=recovery_min_support,
        ...
        limitations=['Convex single-volume roof only; concave footprints/courtyards/extensions unsupported',
          'Small roof details below 7.5 percent support omitted','Input unit assumed meter for Blender display only',
