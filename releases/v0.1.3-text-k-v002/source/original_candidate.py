...
def planes_ransac(p,threshold=.10,min_support=None):
    rng=np.random.default_rng(739)
    rem=np.arange(len(p)); planes=[]; supports=[]
    min_size=max(60,int(len(p)*.075)) if min_support is None else min_support
    for stage in range(8):
        ...
        if best.sum()<min_size or not np.isfinite(coef).all(): break
        centered=q[best,:2]-q[best,:2].mean(axis=0)
        spread=np.linalg.eigvalsh(centered.T@centered/max(1,best.sum()))
        if spread[0]<.01 or np.linalg.norm(coef[:2])>2: break
...
def supported_outline(p,rectangle):
    # Convex completion, not an emptiness claim: require observed edge support.
    hull=MultiPoint(p[:,:2]).convex_hull.simplify(.08,preserve_topology=True)
    if hull.geom_type!='Polygon': return rectangle,'rectangle_fallback'
    poly=np.array(hull.exterior.coords)[:-1]
    ratio=hull.area/Polygon(rectangle).area
    if not (.55<ratio<.94) or not (3<=len(poly)<=16):
        return rectangle,'rectangle_fallback'
    supported=0
    for a,b in zip(poly,np.roll(poly,-1,axis=0)):
        edge=b-a; length=np.linalg.norm(edge)
        t=(p[:,:2]-a)@edge/max(length*length,1e-12)
        distance=np.abs(np.cross(edge,p[:,:2]-a))/max(length,1e-12)
        near=(distance<.20)&(t>=0)&(t<=1)
        bins=np.unique(np.minimum(7,(t[near]*8).astype(int)))
        supported+=int(near.sum()>=6 and len(bins)>=5)
    if supported!=len(poly): return rectangle,'rectangle_fallback'
    if np.sum(np.cross(poly,np.roll(poly,-1,axis=0)))<0: poly=poly[::-1]
    return poly,'observed_edge_supported_convex_hull; unseen extent uncertain'
...
def run(num):
    ...
    recovery='not_needed'
    if len(coef)==0:
        # Fixed 60-point floor rejects small but spatially supported roofs.
        required=max(12,int(np.ceil(.10*len(roof))))
        coef,supports=planes_ransac(roof,min_support=required)
        recovery='adaptive_support'
    if len(coef)==0:
        relaxed=(np.abs(n[:,2])>.50)&(curve<.065)&(p[:,2]>=np.quantile(p[:,2],.25))
        roof=p[relaxed]
        coef,supports=planes_ransac(roof,min_support=max(12,int(np.ceil(.10*len(roof)))))
        recovery='relaxed_normal_and_adaptive_support'
    if len(coef)==0: raise RuntimeError('No spatially supported roof plane')
    ...
    for count in range(1,len(coef)+1):
        for ids in itertools.combinations(range(len(coef)),count):
            ...
            score=float(np.minimum(err,.6).mean()+.004*count)
    ...
    footprint,footprint_rule=supported_outline(p,footprint)
    ...
    metrics=dict(input_points=len(p),roof_candidate_points=len(roof),plane_supports=supports,
        ...
        footprint_rule=footprint_rule,roof_support_recovery=recovery,
        completion_uncertainty='Unseen extent and terrain unknown; no empty-space inference',
        limitations=['Convex single-volume roof only; concave footprints/courtyards/extensions unsupported',
          'Ordinary fit retains 7.5 percent floor; recovery uses max(12, 10 percent); details may be omitted','Input unit assumed meter for Blender display only',
