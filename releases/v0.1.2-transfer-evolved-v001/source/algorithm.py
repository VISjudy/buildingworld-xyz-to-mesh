...
def run(num):
    ...
    recovery=None
    if len(coef)==0:
        # Strict height filtering can exclude an entire quantile-tied roof.
        # Retry only newly admitted ties; keep the original support tests.
        tiedmask=(np.abs(n[:,2])>.55)&(curve<.035)&(p[:,2]>=np.quantile(p[:,2],.25))
        added=int(np.count_nonzero(tiedmask & ~roofmask))
        if added:
            retry_roof=p[tiedmask]
            retry_coef,retry_supports=planes_ransac(retry_roof)
            if len(retry_coef):
                roof=retry_roof; coef=retry_coef; supports=retry_supports
                recovery=dict(operator='failed_fit_quantile_tie_retry',
                    added_quantile_tied_points=added,
                    original_roof_candidate_points=int(np.count_nonzero(roofmask)),
                    scope='input-supported plane retry; no verified terrain or completeness')
    ...
    if recovery is not None: metrics['failure_recovery']=recovery
