...
def run(num):
    ...
    n,curve=normals(p)
    heightmask=p[:,2]>np.quantile(p[:,2],.25)
    height_tolerance=32*np.finfo(float).eps*max(1.,float(np.max(np.abs(p[:,2]))))
    # A constant-height scan has no meaningful lower-height quartile to reject.
    if np.ptp(p[:,2])<=height_tolerance:
        heightmask=np.ones(len(p),dtype=bool)
    roofmask=(np.abs(n[:,2])>.55)&(curve<.035)&heightmask
    roof=p[roofmask]
