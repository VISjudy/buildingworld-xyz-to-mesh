...
def run_batch(source, manifest, output, timeout=180):
    ...
    for s in spec["samples"]:
        ...
        try:
            ...
            cmd=[sys.executable,"-m","harness.adapter","--source",str(frozen/"algorithm.py"),
...
def freeze_release(run, version, knowledge, baseline=None, previous=None):
    ...
    shutil.copytree(run/"source",dest/"source")
    ...
    (dest/"summary.md").write_text(f"# {version}\n\nStatus: frozen research candidate, not an official benchmark result.\n\n"
